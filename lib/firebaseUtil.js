...
const vectorIdRef = db.collection('fileVectors');
...
async function saveFileVectorIdsToFirebase(fileName, vectorIds) {
  try {
    ...
    console.log(`Saved vector IDs for file "${fileName}" to Firestore.`);
  } catch (error) {
...
async function fetchFileVectorIdsFromFirebase(fileName) {
  try {
    ...
    if (!docSnapshot.exists) {
      console.log(`No vector IDs found for file "${fileName}".`);
    ...
    console.log(`Fetched vector IDs for file "${fileName}":`, vectorIds);
    return vectorIds;
  } catch (error) {
