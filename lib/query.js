...
module.exports = { processQuery, processThreads, processFileToPinecone };
...
async function processFileToPinecone(fileName, fileURL) {
    try {
      ...
      if (vectors.length > 0) {
        ...
        console.log(`Successfully upserted ${vectors.length} records for file ${fileName} into Pinecone.`);
      } else {
        console.warn(`No vectors generated for file ${fileName}.`);
      }
  
      return `Success: File ${fileName} processed and upserted to Pinecone.`;
    } catch (error) {
